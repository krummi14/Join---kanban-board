...
/**
 * Handles the splash screen animation after the page has loaded.
 * 
 * Adds a fade-out animation and removes the splash screen element
 * from the DOM after a short delay.
 */
 ...
 * 
 * Removes user-related information from localStorage and sessionStorage
...
/**
 * Updates the user badge in the header with the current user's initials.
 * 
 * If the logged-in user is not a guest, the initials are generated
 * from the user name and rendered into the header element.
 */
 ...
 * Returns a single "G" for guest users.
 * 
 * @param {string} fullName - The full name of the current user.
 * @returns {string} The generated initials in uppercase letters.
 */
...
/**
 * Displays or clears a validation error message.
 * 
 * Adds the "show" class when an error message exists
 * and removes it when the message is empty.
 * 
...
/**
 * Renders the Add Task form template into the page container.
 * 
 * Loads the generated Add Task HTML template and inserts it
 * into the corresponding container element.
 * 
 * @param {string} path - The current path or context for the task form.
 */
...
/**
 * Toggles the mobile logout menu visibility.
 * 
 * Opens and closes the responsive logout menu
 * while locking or restoring page scrolling.
 */
function addMenu() {
    if (menuStatus === "on") {
