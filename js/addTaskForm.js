...
function createController(context) {
  return { 
    ...
    // 👇 DAS HIER HINZUFÜGEN
    setAssigneesFromTask: (task) => {
      const ids = task.assignees?.map(a => a.id) || [];

      context.state.selectedAssignees = ids;
...
async function renderAssigneeContacts(context) {
  ...
  try {
    ...
    // ✅ HIER LOGGEN
    console.log("ASSIGNEE CONTACTS:", context.state.assigneeContacts);
  ...
  // ✅ DEBUG HTML
  console.log("ASSIGNEE MENU HTML:", menu.innerHTML);
...
//board overlay edit 
function setAssigneesFromTask(context, task) {
  const ids = task.assignees?.map(a => a.id) || [];
